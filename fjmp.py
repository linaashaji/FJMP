...
parser.add_argument(
    "--mode",
    choices=["train", "eval", "eval_constant_velocity"],
    help="running mode : (train, eval, eval_constant_velocity)",
    default="train",
)
parser.add_argument(
    "--dataset",
    choices=["interaction", "argoverse2"],
    help="dataset : (interaction, argoverse2)",
    default="interaction",
)
parser.add_argument(
    "--config_name",
    default="dev",
    help="a name to indicate the log path and model save path",
)
parser.add_argument(
    "--num_edge_types",
    default=3,
    type=int,
    help="3 types: no-interaction, a-influences-b, b-influences-a",
)
parser.add_argument(
    "--h_dim",
    default=128,
    type=int,
    help="dimension for the hidden layers of MLPs. Note that the GRU always has h_dim=256",
)
parser.add_argument(
    "--num_joint_modes", default=6, type=int, help="number of scene-level modes"
)
parser.add_argument(
    "--num_proposals", default=15, type=int, help="number of proposal modes"
)
parser.add_argument("--batch_size", default=16, type=int)
parser.add_argument(
    "--max_epochs", default=50, type=int, help="maximum number of epochs"
)
parser.add_argument("--lr", default=1e-3, type=float, help="initial learning rate")
parser.add_argument(
    "--decoder",
    choices=["dagnn", "lanegcn"],
    help="decoder architecture : (dagnn, lanegcn)",
    default="dagnn",
)
parser.add_argument(
    "--num_heads",
    default=1,
    type=int,
    help="number of heads in multi-head attention for decoder attention.",
)
parser.add_argument(
    "--learned_relation_header",
    action="store_true",
    help="if true, network learns+predicts interaction graph with interaction graph predictor. Otherwise, ground-truth pseudolabels are used.",
)
parser.add_argument(
    "--gpu_start",
    default=0,
    type=int,
    help="gpu device i, where training will occupy gpu device i,i+1,...,i+n_gpus-1",
)
parser.add_argument(
    "--n_mapnet_layers", default=2, type=int, help="number of MapNet blocks"
)
parser.add_argument(
    "--n_l2a_layers", default=2, type=int, help="number of L2A attention blocks"
)
parser.add_argument(
    "--n_a2a_layers", default=2, type=int, help="number of A2A attention blocks"
)
parser.add_argument(
    "--resume_training", action="store_true", help="continue training from checkpoint"
)
parser.add_argument(
    "--proposal_coef", default=1, type=float, help="coefficient for proposal losses"
)
parser.add_argument(
    "--rel_coef",
    default=100,
    type=float,
    help="coefficient for interaction graph prediction losses.",
)
parser.add_argument(
    "--proposal_header",
    action="store_true",
    help="add proposal multitask training objective?",
)
parser.add_argument(
    "--two_stage_training", action="store_true", help="train relation predictor first?"
)
parser.add_argument(
    "--training_stage",
    default=1,
    type=int,
    help="1 or 2. Which training stage in 2 stage training?",
)
parser.add_argument(
    "--ig",
    choices=["sparse", "dense", "m2i"],
    help="which interaction graph pseudolabels to use",
    default="sparse",
)
parser.add_argument(
    "--focal_loss",
    action="store_true",
    help="use multiclass focal loss for relation header?",
)
parser.add_argument(
    "--gamma", default=5, type=float, help="gamma parameter for focal loss."
)
parser.add_argument(
    "--weight_0", default=1.0, type=float, help="weight of class 0 for relation header."
)
parser.add_argument(
    "--weight_1", default=2.0, type=float, help="weight of class 1 for relation header."
)
parser.add_argument(
    "--weight_2", default=4.0, type=float, help="weight of class 2 for relation header."
)
parser.add_argument(
    "--teacher_forcing",
    action="store_true",
    help="use teacher forcing of influencer future predictions?",
)
parser.add_argument(
    "--scheduled_sampling",
    action="store_true",
    help="use linear schedule curriculum for teacher forcing of influencer future predictions?",
)
parser.add_argument(
    "--eval_training", action="store_true", help="run evaluation on training set?"
)
parser.add_argument(
    "--supervise_vehicles",
    action="store_true",
    help="supervise only vehicles in loss function (for INTERACTION)?",
)
parser.add_argument(
    "--train_all",
    action="store_true",
    help="train on both the train and validation sets?",
)
parser.add_argument(
    "--no_agenttype_encoder",
    action="store_true",
    help="encode agent type in FJMP encoder? Only done for Argoverse 2 as INTERACTION only predicts vehicle trajectories.",
)
...
class FJMP(torch.nn.Module):
    ...
    def build(self):
        ...
        ) or not self.two_stage_training:
            if self.decoder == "dagnn":
                self.trajectory_decoder = FJMPAttentionTrajectoryDecoder(
                    self.config
                ).to(dev)
    ...
    def process(self, data):
        ...
        if self.dataset == "argoverse2":
            ...
            agentcategories = torch.cat(agentcategories, 0)[
                :, self.observation_steps - 1, 0
            ]
        ...
        orig = [
            x.view(1, 2)
            for j, x in enumerate(data["orig"])
            for i in range(num_actors[j])
        ]
        orig = torch.cat(orig, 0)

        rot = [
            x.view(1, 2, 2)
            for j, x in enumerate(data["rot"])
            for i in range(num_actors[j])
        ]
        rot = torch.cat(rot, 0)

        theta = torch.Tensor(
            [x for j, x in enumerate(data["theta"]) for i in range(num_actors[j])]
        )
    ...
    def _train(
        ...
        for epoch in range(start_epoch, self.max_epochs + 1):
            ...
            self.train()
            tot_log = self.num_train_samples // (self.batch_size)
            ...
            if self.scheduled_sampling:
                ...
                prop_ground_truth = 1.0
            else:
                prop_ground_truth = 0.0
            ...
            for i, data in enumerate(train_loader):
                ...
                dgl_graph = self.feature_encoder(
                    dgl_graph,
                    dd["feats"][:, : self.observation_steps],
                    dd["agenttypes"],
                    dd["actor_idcs"],
                    dd["actor_ctrs"],
                    dd["lane_graph"],
                ...
                if self.two_stage_training and self.training_stage == 2:
                    stage_1_graph = self.build_stage_1_graph(
                        dgl_graph,
                        dd["feats"][:, : self.observation_steps],
                        dd["agenttypes"],
                        dd["actor_idcs"],
                ...
                res = self.forward(
                    ...
                    dd["batch_idxs_edges"],
                    dd["actor_ctrs"],
                    prop_ground_truth=prop_ground_truth,
                    eval=False,
                )
                ...
                optimizer.zero_grad()
                loss.backward()
                accum_gradients = accumulate_gradients(
                    accum_gradients, self.named_parameters()
                )
                optimizer.step()
                ...
                if i % 100 == 0:
                    print(
                        ...
                        "lr={:.3e}".format(cur_lr),
                        "rel_coef={:.1f}".format(self.rel_coef),
                        "\t".join(
                ...
                if self.eval_training:
                    ...
                    gt_ctrs_all.append(
                        (
                            torch.matmul(dd["ctrs"].unsqueeze(1), dd["rot"]).squeeze(1)
                            + dd["orig"]
                        )
                        .detach()
                        .cpu()
            ...
            if self.eval_training:
                ...
                if self.dataset == "interaction":
                    eval_agent_mask = np.concatenate(agenttypes_all, axis=0)[
                        :, 1
                    ].astype(bool)
                else:
                    eval_agent_mask = np.ones(
                        np.concatenate(agenttypes_all, axis=0)[:, 1].shape
                    ).astype(bool)
                ...
                eval_results = calc_metrics(
                    results, self.config, mask, identifier="reg"
                )
                ...
                print(
                    ...
                    "\t".join(
                        [
                            f"{k}: {v}" if type(v) is np.ndarray else f"{k}: {v:.3f}"
    ...
    def _eval(self, val_loader, epoch):
        ...
        tot = 0
        with torch.no_grad():
            tot_log = self.num_val_samples // (self.batch_size)
        ...
        if self.mode == "train":
            ...
            all_val_eval_results = calc_metrics(
                results, self.config, mask, identifier="reg"
            )
            ...
            if (not self.two_stage_training) or (
                ...
                val_eval_results = calc_metrics(
                    results, self.config, mask, identifier="int"
                )
                val_eval_results = sync(val_eval_results, self.config, comm)
                ...
                hardness_3_mask = np.load("fde_3_{}.npy".format(self.dataset)).astype(
                    bool
                )
                ...
                hardness_5_mask = np.load("fde_5_{}.npy".format(self.dataset)).astype(
                    bool
                )
                ...
                if self.dataset == "argoverse2":
                    ...
                    hardness_3_mask = np.load(
                        "fde_3_{}.npy".format(self.dataset)
                    ).astype(bool)
                    ...
                    hardness_5_mask = np.load(
                        "fde_5_{}.npy".format(self.dataset)
                    ).astype(bool)
    ...
    def init_dgl_graph(
        self, batch_idxs, ctrs, orig, rot, agenttypes, world_locs, has_preds
    ):
        ...
        for ii in range(n_scenarios):
            ...
            graph.ndata["ground_truth_futures"] = world_locs[batch_idxs == ii][
    ...
    def build_stage_1_graph(
        self, graph, x, agenttypes, actor_idcs, actor_ctrs, lane_graph
    ):
        all_edges = [x.unsqueeze(1) for x in graph.edges("uv")]
        all_edges = torch.cat(all_edges, 1)

        stage_1_graph = dgl.graph(
            (all_edges[:, 0], all_edges[:, 1]), num_nodes=graph.num_nodes()
        )
        ...
        stage_1_graph = self.pretrained_relation_header.feature_encoder(
            stage_1_graph, x, agenttypes, actor_idcs, actor_ctrs, lane_graph
        )
    ...
    def forward(
        ...
        if self.learned_relation_header:
            ...
            if not self.two_stage_training:
                edge_probs = torch.nn.functional.one_hot(
                    ig_dict["ig_labels"].to(dev).long(), self.num_edge_types
                )
            elif self.two_stage_training and self.training_stage == 2:
                prh_logits = self.pretrained_relation_header.relation_header(
                    stage_1_graph
                )
        ...
        if self.learned_relation_header or (
            self.two_stage_training and self.training_stage == 2
        ):
        ...
        if self.learned_relation_header:
            res[
                "edge_logits"
            ] = edge_logits.float()  # edge probabilities for computing BCE loss
    ...
    def get_loss(
        ...
        if self.proposal_header:
            ...
            has_preds_mask = (
                has_preds_mask.expand(
                    has_preds_mask.shape[0],
                    has_preds_mask.shape[1],
                    self.num_proposals,
                    2,
                )
                .bool()
        ...
        if self.learned_relation_header:
            if (not self.two_stage_training) or (
                self.two_stage_training and self.training_stage == 1
            ):
                if self.focal_loss:
                    ce_loss = FocalLoss(
                        weight=torch.Tensor(
                            [self.weight_0, self.weight_1, self.weight_2]
                        ).to(dev),
                        gamma=self.gamma,
                        reduction="mean",
                    )
        ...
        if not self.two_stage_training:
            ...
            loss_dict = {"total_loss": loss, "loss_reg": loss_reg}
            ...
            if self.training_stage == 1:
                ...
                loss_dict = {"total_loss": loss, "loss_rel": self.rel_coef * loss_rel}
    ...
    def _eval_constant_velocity(self, val_loader, epoch):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(val_loader):
                ...
                for j in range(N):
                    counted_interaction = (dd["has_preds"][j, -1] == 1) and (
                        dd["agenttypes"][j, 1] == 1
                    )
                    counted_argoverse2 = dd["has_preds"][j, -1] == 1

                    if (self.dataset == "interaction" and counted_interaction) or (
                        self.dataset == "argoverse2" and counted_argoverse2
                    ):
                        # interactive case
                    ...
                    final_x, final_y = estimate_constant_velocity(
                        x[j],
                        self.prediction_steps,
                        dd["has_obs"][j, : self.observation_steps],
                    )
                    gt_final_x, gt_final_y = (
                        dd["gt_locs"][j, -1, 0],
                        dd["gt_locs"][j, -1, 1],
                    )
...
if __name__ == "__main__":
    ...
    if args.dataset == "interaction":
        ...
        config["tracks_train_reformatted"] = os.path.join(
            config["dataset_path"], "train_reformatted"
        )
        config["tracks_val_reformatted"] = os.path.join(
            config["dataset_path"], "val_reformatted"
        )
        config["num_scales"] = 4
        ...
        if config["train_all"]:
            config["preprocess_train"] = os.path.join(
                config["dataset_path"], "preprocess", "train_all_interaction"
            )
        else:
            config["preprocess_train"] = os.path.join(
                config["dataset_path"], "preprocess", "train_interaction"
            )
        config["preprocess_val"] = os.path.join(
            config["dataset_path"], "preprocess", "val_interaction"
        )
        config["batch_size"] = args.batch_size

        if config["mode"] == "train":
            dataset = InteractionDataset(
                config, train=True, train_all=config["train_all"]
            )
        ...
        if config["train_all"]:
            config["preprocess_train"] = os.path.join(
                config["dataset_path"], "preprocess", "train_all_argoverse2"
            )
        else:
            config["preprocess_train"] = os.path.join(
                config["dataset_path"], "preprocess", "train_argoverse2"
            )
        config["preprocess_val"] = os.path.join(
            config["dataset_path"], "preprocess", "val_argoverse2"
        )
        config["batch_size"] = args.batch_size
    ...
    if args.mode == "train":
        ...
        if model.two_stage_training and model.training_stage == 1:
            with open(
                os.path.join(config["log_path"], "config_stage_1.pkl"), "wb"
            ) as f:
        ...
        if model.two_stage_training and model.training_stage == 2:
            with open(
                os.path.join(config["log_path"], "config_stage_1.pkl"), "rb"
            ) as f:
        ...
        starting_epoch = 1
        val_best, ade_best, fde_best, val_edge_acc_best = np.inf, np.inf, np.inf, 0.0
        # resume training from checkpoint
        if config["resume_training"]:
            if (not model.two_stage_training) or (
                model.two_stage_training and model.training_stage == 2
            ):
                (
                    optimizer,
                    starting_epoch,
                    ...
                    ade_best,
                    fde_best,
                ) = model.load_for_train(optimizer)
            else:
                (
                    optimizer,
                    starting_epoch,
                    val_edge_acc_best,
                ) = model.load_for_train_stage_1(optimizer)
