...
class InteractionDataset(Dataset):
    def __init__(self, config, train=True, train_all=False):
        ...
        if not os.path.isfile(
            os.path.join(self.config["dataset_path"], self.mapping_filename)
        ):
            ...
            for csv_file in os.listdir(self.tracks):
                ...
                with open(csv_path, "r") as src:
                    ...
                    for scenario in scenarios:
                        ...
                        track_reformatted = (
                            csv_file[:-4] + "_{}".format(int(float(scenario))) + ".csv"
                        )
                        track_reformatted_path = os.path.join(
                            self.tracks_reformatted, track_reformatted
                        )
            ...
            with open(
                os.path.join(self.config["dataset_path"], self.mapping_filename), "wb"
            ) as f:
                pickle.dump(mapping, f)

        # load mapping dictionary
        with open(
            os.path.join(self.config["dataset_path"], self.mapping_filename), "rb"
        ) as f:
    ...
    def read_interaction_data(self, idx):
        ...
        vels = np.concatenate(
            (df.vx.to_numpy().reshape(-1, 1), df.vy.to_numpy().reshape(-1, 1)), 1
        )

        psirads = df.psi_rad.to_numpy().reshape(-1, 1)

        agenttypes = df.agent_type
        agenttypes = np.array([1 if x == "car" else 0 for x in agenttypes]).reshape(
            -1, 1
        )

        shapes = np.concatenate(
            (df.length.to_numpy().reshape(-1, 1), df.width.to_numpy().reshape(-1, 1)), 1
        )
    ...
    def get_interaction_labels_sparse(
        self,
        idx,
        ctrs,
        feat_locs,
        ...
        for i in range(N):
            ...
            circle_lists.append(
                return_circle_list(
                    traj_i[:, 0], traj_i[:, 1], length, width, psirad_i[:, 0]
                )
            )
        ...
        for a in range(1, N):
            for b in range(a):
                ...
                dist = np.expand_dims(
                    np.expand_dims(circle_list_a, axis=1), axis=2
                ) - np.expand_dims(np.expand_dims(circle_list_b, axis=0), axis=3)
                ...
                candidate_reactors = coll_ids_sorted[
                    coll_ids_sorted[:, 0] == conflict_time_influencer
                ][:, 1]
                conflict_time_reactor = candidate_reactors.min()
                conflict_time_reactor_id = np.argmin(candidate_reactors)

                a_is_influencer = (
                    coll_ids_argsorted[influencer_mask][conflict_time_reactor_id][0]
                    == 0
                )
                if a_is_influencer:
                ...
                # b is the influencer
                    ...
                    if np.linalg.norm(feat_vels[a][min_a], ord=2) > np.linalg.norm(
                        feat_vels[b][min_b], ord=2
                    ):
                        labels[a, b] = 1
                    elif np.linalg.norm(feat_vels[a][min_a], ord=2) < np.linalg.norm(
                        feat_vels[b][min_b], ord=2
                    ):
                        labels[b, a] = 1
                    else:
    ...
    def get_interaction_labels_dense(
        self,
        idx,
        ctrs,
        feat_locs,
        ...
        for a in range(1, N):
            for b in range(a):
                ...
                dist_ab = np.sqrt(
                    np.sum((traj_a_expanded - traj_b_expanded) ** 2, axis=2)
                )
                ...
                if min_a < min_b:
                    ...
                    # if both reach the conflict point at the same timestep, the influencer is the vehicle with the higher velocity @ the conflict point.
                    if np.linalg.norm(feat_vels[a][min_a + 10], ord=2) > np.linalg.norm(
                        feat_vels[b][min_b + 10], ord=2
                    ):
                        labels[a, b] = 1
                    elif np.linalg.norm(
                        feat_vels[a][min_a + 10], ord=2
                    ) < np.linalg.norm(feat_vels[b][min_b + 10], ord=2):
                        labels[b, a] = 1
                    else:
                        labels[a, b] = 0
                        labels[b, a] = 0
    ...
    def get_obj_feats(self, data, idx):
        ...
        # center on the agent closest to the centroid
            ...
            for i in range(len(data["trajs"])):
                if (
                    8 in data["steps"][i]
                    and 9 in data["steps"][i]
                    and 39 in data["steps"][i]
                ):
                    present_index = list(data["steps"][i]).index(9)
                    scored_ctrs.append(
                        np.expand_dims(data["trajs"][i][present_index], axis=0)
                    )
            ...
            dist_to_centroid = np.linalg.norm(
                scored_ctrs - np.expand_dims(centroid, axis=0), ord=2, axis=-1
            )
        ...
        for traj, step, vel, psirad, shape, agenttype in zip(
            data["trajs"],
            data["steps"],
            data["vels"],
            data["psirads"],
            ...
            post_psirad = (
                np.nan_to_num(psirad[future_mask]) * post_agenttype
            )  # 0 out psirad if not a car
            ...
            feat_shape[step] = (
                np.nan_to_num(shape) * feat_agenttype[step]
            )  # 0 out if not a car
            ...
            # velocity is already in se(2) transformed coordinates
                ...
                for j in range(len(psirad)):
                    speed_j = math.sqrt(
                        vel_transformed[j, 0] ** 2 + vel_transformed[j, 1] ** 2
                    )
                    if speed_j == 0:
                        psirad[j, 0] = 0.0
                    else:
                        psirad[j, 0] = round(
                            sign_func(vel_transformed[j, 1])
                            * math.acos(vel_transformed[j, 0] / speed_j),
                            3,
                        )
        ...
        ig_labels_dense = self.get_interaction_labels_dense(
            idx,
            ctrs,
            feat_locs,
            feat_vels,
        ...
        ig_labels_sparse = self.get_interaction_labels_sparse(
            idx,
            ctrs,
            feat_locs,
            feat_vels,
    ...
    def get_lane_graph(self, data):
        ...
        for ll in map.laneletLayer:
            ...
            centerline, _ = compute_midpoint_line(
                left_boundary,
                right_boundary,
                min(10, max(left_boundary.shape[0], right_boundary.shape[0])),
            )
            centerline = copy.deepcopy(centerline)

            # process lane centerline in same way as agent trajectories
            centerline = np.matmul(
                data["rot"], (centerline - data["orig"].reshape(-1, 2)).T
            ).T
            left_boundary = np.matmul(
                data["rot"], (left_boundary - data["orig"].reshape(-1, 2)).T
            ).T
            right_boundary = np.matmul(
                data["rot"], (right_boundary - data["orig"].reshape(-1, 2)).T
            ).T

            num_segs = len(centerline) - 1
            # locations between the centerline segments
            ctrs.append(
                np.asarray((centerline[:-1] + centerline[1:]) / 2.0, np.float32)
            )
    ...
    def __getitem__(self, idx):
        if self.config["preprocess"]:
            data = np.load(
                os.path.join(self.preprocess_path, "{}.p".format(idx)),
                allow_pickle=True,
            )

            new_data = dict()
            for key in [
                "idx",  # unique integer assigned to each scene (assigned when building mapping dictionary)
...
class InteractionTestDataset(Dataset):
    def __init__(self, config):
        ...
        self.n_samples = 2644
        self.tracks_reformatted = os.path.join(
            config["dataset_path"], "test_reformatted"
        )
        self.preprocess_path = os.path.join(
            config["dataset_path"], "preprocess", "fjmp_test_nov6"
        )
    ...
    def read_interaction_data(self, idx):
        ...
        (
            ...
            ctx_psirads,
            ctx_track_ids,
            ctx_agent_to_predicts,
            ctx_interesting_agents,
            ctx_shapes,
            ctx_agenttypes,
        ) = ([], [], [], [], [], [], [], [], [])
        for key in keys:
    ...
    def get_obj_feats(self, data, idx):
        ...
        for (
            ...
            data["psirads"],
            data["track_ids"],
            data["agent_to_predicts"],
            data["interesting_agents"],
            data["shapes"],
            data["agenttypes"],
        ):
            if 9 not in step:
